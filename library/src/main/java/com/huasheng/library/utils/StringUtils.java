 ...
 * @Package com.custom.libs.utils
 * @Description: TODO(String基本处理方式 Utility class to peform common String manipulation algorithms.)
 * @date 2016/3/14
 * @time 16:49
 */
...
public class StringUtils {
    ...
    /**
     * 是否为中文 不能为全角标点符号判定、全角英字、全角数字、全角空格
     *
     * @param c
     * @return
     */
    public static boolean isChinese(char c) {
        ...
        if (c >= 0x0391 && c <= 0xffe5 && !HalfAndFullHornSymbolsUtils.isSingleFullHornSymbol(c) && !HalfAndFullHornSymbolsUtils.isSingleFullHornAlpha(c)
                && !HalfAndFullHornSymbolsUtils.isSingleFullHornDigit(c)) {
            isChinese = true;
        } else {
            isChinese = false;
        }
    ...
    /**
     * 只能输入半角标点符号、半角英数和半角"@"符号
     *
     * @return 判定结果 true:验证合法
     */
    public static boolean isStringValidate(final String str) {
        for (int i = str.length() - 1; 0 <= i; i--) {
            char c = str.charAt(i);
            if (!HalfAndFullHornSymbolsUtils.isSingleByteDigit(c) && !HalfAndFullHornSymbolsUtils.isSingleByteSymbol(c) && !HalfAndFullHornSymbolsUtils.isSingleByteAlpha(c)) {
                if (c != "@".charAt(0)) {
                    return false;
                }
            }
    ...
    /**
     * 只能输入半角标点符号、半角英数、半角"@"符号和中文文字
     *
     * @return 判定结果 true:验证合法
     */
    public static boolean isStringValidate_2(final String str) {
        for (int i = str.length() - 1; 0 <= i; i--) {
            char c = str.charAt(i);
            if (!HalfAndFullHornSymbolsUtils.isSingleByteDigit(c) && !HalfAndFullHornSymbolsUtils.isSingleByteSymbol(c) && !HalfAndFullHornSymbolsUtils.isSingleByteAlpha(c)) {
                if (c != "@".charAt(0) && !isChinese(c)) {
                    return false;
                }
            }
        }
